...
flowchart TD
    ...
    X_x --> interp_coarse_X_xt_X_x --> X_x_fine --> bus_X_fine --> X_fine --> f --> delta_LST_fine_pred --> add --> delta_LST_fine_pred_corr --> destandardisation --> LST_fine_pred_corr

    interp_coarse_X_xt_X_x --> X_x_coarse --> bus_X_coarse --> X_coarse --> f --> delta_LST_coarse_pred --> subtract --> res_coarse --> interp_fine_res --> res_fine_pred --> add

    X_xt_fine --> interp_coarse_X_xt_X_x --> X_xt_coarse --> standardisation --> delta_X_xt_coarse --> bus_X_coarse

    X_xt_fine --> standardisation --> delta_X_xt_fine --> bus_X_fine



    LST_coarse --> standardisation --> delta_LST_coarse --> subtract
    ...
    delta_X_xt_coarse["$$\delta X_{xt,\mathrm{coarse}}$$"]
    delta_X_xt_fine["$$\delta X_{xt,\mathrm{fine}}$$"]
    LST_coarse["$$\mathrm{LST}_{\mathrm{coarse}}$$"]
    LST_fine_pred_corr["$$\hat{\mathrm{LST}}_{\mathrm{fine},\mathrm{corr}}$$"]
    delta_LST_coarse["$$\delta\mathrm{LST}_{\mathrm{coarse}}$$"]
    delta_LST_coarse_pred["$$\hat{\delta\mathrm{LST}}_{\mathrm{coarse}}$$"]
    delta_LST_fine_pred["$$\hat{\delta\mathrm{LST}}_{\mathrm{fine}}$$"]
    delta_LST_fine_pred_corr["$$\hat{\delta\mathrm{LST}}_{\mathrm{fine},\mathrm{corr}}$$"]
    ...
    standardisation["$$\begin{align*}\text{Timestamp-specific} \\\\ \text{standardisation}\end{align*}$$"]@{ shape: rounded}
    destandardisation["$$\begin{align*}\text{Timestamp-specific} \\\\ \text{de-standardisation}\end{align*}$$"]@{ shape: rounded}
    bus_X_coarse@{shape: fork}
    bus_X_fine@{shape: fork}

    %% Styling
    style X_x stroke:#4A4A4A,stroke-width:4px
    style X_t stroke:#4A4A4A,stroke-width:4px
    ...
    style delta_X_xt_coarse stroke:#4A4A4A,stroke-width:2px
    style delta_X_xt_fine stroke:#4A4A4A,stroke-width:2px
    style LST_coarse stroke:#4A4A4A,stroke-width:4px
    style LST_fine_pred_corr stroke:#4A4A4A,stroke-width:2px
    style delta_LST_coarse stroke:#4A4A4A,stroke-width:2px
    style delta_LST_coarse_pred stroke:#4A4A4A,stroke-width:2px
    style delta_LST_fine_pred stroke:#4A4A4A,stroke-width:2px
    style delta_LST_fine_pred_corr stroke:#4A4A4A,stroke-width:2px
    ...
    style standardisation fill:#c4c4c4,stroke:#4A4A4A,stroke-width:2px
    style destandardisation fill:#c4c4c4,stroke:#4A4A4A,stroke-width:2px
